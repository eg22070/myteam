<x-app-layout>
 ...
 Edit teams information</a>
            ...
            <h3 class="font-semibold text-l text-gray-800 leading-tight">
                ...
                <div class=" bg-white border border-sky-400 rounded-md shadow-sm sm:rounded-lg p-6 text-gray-900">
            <h2 class=" hover-underline-animation border-sky-400 rounded-md font-semibold text-xl text-gray-800 leading-tight"><a href="{{route('comment', ['id' => $teams->id]) }}"><u>View coach comments</u></a></h2>
</br> @can('is-coach-or-owner')
            <a href="{{route('comment.create', ['id' => $teams->id]) }}"
 class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
 Add comment</a>
@endcan
            </div>
